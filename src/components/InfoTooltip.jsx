import checkPath from '../images/check.svg';
import errorPath from '../images/error.svg';

function InfoTooltip (onClose) {

    return (
        <div className='popup popup_is-opened'> {/*Попап на всю страницу*/}
            <div className='popup-container'> {/*Окно оповещения*/}
                <button className='close-icon' onClick={onClose}></button> {/*Кнопка закрытия*/}
                <div className="info-popup">
                    <img className="info-popup__image" src={checkPath} alt='галочка'/>
                    <p className="info-popup__caption">Вы успешно зарегистрировались!</p>
                </div>
            </div>
        </div>
    )
}

export default InfoTooltip;